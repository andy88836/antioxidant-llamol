...
class Sampler:
    ...
    def get_context(
        ...
        if "hbd" in context_col:
            if self.sample_range == "UNC" or self.sample_range == "1D":
                context = torch.randint(
                    0, 10, (num_examples,), device=self.device, dtype=torch.float
                )
            elif self.sample_range == "2D":
                context = torch.tensor(
                    np.random.choice([1, 2, 3, 4], (num_examples,)),
                    device=self.device,
                    dtype=self.ptdtype,
                )
            elif self.sample_range == "3D" or self.sample_range == "TOK":
            ...
            output_dict["context"]["hbd"] = context
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ckpt_path",
        type=str,
        default=os.path.join(os.path.dirname(__file__), "out", "llama2-M-Full-RSS-Canonical.pt"),
    ...
    parser.add_argument(
        ...
        type=str,
        default="UNC",
        help="Is either Unc,1D,2D,3D or TOK determines the range of logp,sascore ,hbd and mol_weight",
    )
    ...
    if args.cmp_dataset_path is not None:
        ...
        comp_context_dict = {
            c: df_comp[c].to_numpy() for c in ["logp", "sascore", "hbd"]
        }
    ...
    # output_file = os.path.join(self.out_dir, "generated_molecules.txt")
    ...
    # 确定生成条件
    condition_labels = [key for key in ["logp", "sascore", "hbd"] if
                        key in context and any(context[key][i] != "N/A" for i in range(len(smiles)))]
    condition_str = "_".join(condition_labels) if condition_labels else "unconditional"

    # 构建文件名
    output_file_path = os.path.join("D:\llamol\generate_output",
                                    f"{save_time}_generated_molecules_with_properties_{condition_str}.csv")
    ...
    with open(output_file_path, "w", newline='') as csvfile:
        fieldnames = ["SMILES", "logp", "sascore", "hbd"]
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        writer.writeheader()
        for i, s in enumerate(smiles):
            row = {"SMILES": s}
            for key in fieldnames[1:]:  # 跳过 "SMILES"
                if key in context:
                    row[key] = context[key][i].item()
                else:
                    row[key] = "N/A"
            writer.writerow(row)

    logger.info(f"Generated molecules with properties saved to {output_file_path}")
    # output_file_path = os.path.join("D:\\llamol\\generate_output",
    #                                 f"{save_time}_generated_molecules_with_properties.csv")
    #
    # with open(output_file_path, "w") as f:
    #     for i, s in enumerate(smiles):
    #         logp = context["logp"][i].item() if "logp" in context else "N/A"
    #         sascore = context["sascore"][i].item() if "sascore" in context else "N/A"
    #         # mol_weight = context["mol_weight"][i].item() if "mol_weight" in context else "N/A"
    #         hbd = context["hbd"][i].item() if "hbd" in context else "N/A"
    #         f.write(f"SMILES: {s}, logP: {logp}, SAScore: {sascore}, HBD: {hbd}\n")
    #     logger.info(f"Generated molecules with properties saved to {output_file_path}")
