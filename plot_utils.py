...
def calcContextHBD(smiles: List[str]):
    hbd = []
    for smi in smiles:
        mol = Chem.MolFromSmiles(smi)
        hb = rdMolDescriptors.CalcNumHBD(mol)
        hbd.append(hb)
    return np.array(hbd)
...
def plot_1D_condition(
    ...
    for con_idx, con_col in enumerate(context_col):
        ...
        if con_col == "mol_weight":
            ...
            predicted_context = calcContextHBD(generated_smiles)
        elif con_col == "logp":
...
def plot_unconditional(
    ...
    context_cols: List[str] = ["logp", "sascore", "hbd"],
    fontsize = 17
):
